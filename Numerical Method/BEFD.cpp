...
#include "BEFD.hpp"
BEFD::BEFD(double _S0, double _K, double _D, double _U,double _r, double _q, double _T, double _sigma, int _M, double _alpha )
{
...
double BEFD::f(int i)
{
    ...
    double x = x_left + (x_right-x_left)/(double)N * i;
...
// 边界条件修改
double BEFD::g_left(int j)
{
    double t = t_final/(double) M *(double) j;
    //2 是可修改变量
    return 0*exp(a * x_left + b * t);
}


//边界条件修改
double BEFD::g_right(int j)
{
    double t = t_final / (double)M * (double)j;
    
    //call
    return 0 * exp(x_right * a + b * t);
    //put
    //return 0;
}
//***************************************************************************************************************
...
double BEFD::Theta()
{
    ...
    cout<<vi1t<<endl<<vi1<<endl;
    return (vso-vst)/(T/M);
}

void BEFD :: ForwardEuler()
{
...
void BEFD :: BackwardEuler(string method)
{
    Matrix<double> a;
...
void BEFD :: CrankNicolson(string method)
{
    Matrix<double> a,b1;
